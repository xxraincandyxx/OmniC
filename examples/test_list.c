...
// Define the structure that will be stored in the list.
...
int main(void) {
  ...
  // Add elements using emplace macros.
  // Note the use of designated initializers (C99+).
  printf("Adding elements...\n");
  oc_list_emplace_back(&my_list, user_data_t, link, .id = 101, .name = "Able");
  oc_list_emplace_back(&my_list, user_data_t, link, .id = 112, .name = "Bank");
  oc_list_emplace_front(&my_list, user_data_t, link, .id = 123,
                        .name = "Castle");
  ...
  // Demonstrate safe iteration while erasing.
  ...
  // Destroy the list, freeing all remaining elements.
